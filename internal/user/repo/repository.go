...
// GetManagedUsers returns all managed (admin-controlled) accounts in a circle.
func (r *UserRepository) GetManagedUsers(c context.Context, circleID int) ([]*uModel.User, error) {
	var users []*uModel.User
	if err := r.db.WithContext(c).
		Where("circle_id = ? AND user_type = ?", circleID, uModel.UserTypeManaged).
...
// UpdateManagedUser updates editable fields of a managed account (non-empty values only).
func (r *UserRepository) UpdateManagedUser(c context.Context, userID int, username, displayName string) error {
...
func (r *UserRepository) GetUserByToken(c context.Context, token string) (*uModel.UserDetails, error) {
	...
	// First, fetch the API token to check if it's expired
	if err := r.db.WithContext(c).Where("token = ?", token).First(&apiToken).Error; err != nil {
		return nil, err
	}

...
// CreateManagedUserRequest is the request body for creating a managed account.
...
// UpdateManagedUserRequest is the request body for editing a managed account.
...
// ManagedUserResponse is the API response shape for managed accounts.
...
// isManagedInCircle verifies a user exists, is managed-type, and belongs to the circle.
func (h *Handler) isManagedInCircle(c *gin.Context, userID, circleID int) (*uModel.User, error) {
	u, err := h.userRepo.GetUserByID(c, userID)
	if err != nil {
		return nil, fmt.Errorf("managed user not found")
	}
	if u.UserType != uModel.UserTypeManaged {
		return nil, fmt.Errorf("user is not a managed account")
	}
	if u.CircleID != circleID {
		return nil, fmt.Errorf("managed user does not belong to your circle")
	}
	return u, nil
}

// isCircleAdmin returns true when currentUser is an admin of their circle.
func (h *Handler) isCircleAdmin(c *gin.Context, circleID, userID int) (bool, error) {
...
func (h *Handler) createManagedUser(c *gin.Context) {
	...
	admin, err := h.isCircleAdmin(c, currentUser.CircleID, currentUser.ID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to check permissions"})
		return
	}
	if !admin {
		c.JSON(http.StatusForbidden, gin.H{"error": "Only circle admins can create managed accounts"})
		return
	}
	...
	managedUser := &uModel.User{
		Username:    req.Username,
		DisplayName: html.EscapeString(displayName),
		Password:    encodedPassword,
		CircleID:    currentUser.CircleID,
		UserType:    uModel.UserTypeManaged,
		Provider:    uModel.AuthProviderDonetick,
		CreatedAt:   time.Now().UTC(),
		UpdatedAt:   time.Now().UTC(),
	}

	createdUser, err := h.userRepo.CreateUser(c, managedUser)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create managed user"})
		return
	}
	...
	if err := h.circleRepo.AddUserToCircle(c, &cModel.UserCircle{
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to add managed user to circle"})
		return
	}

	c.JSON(http.StatusCreated, gin.H{"res": ManagedUserResponse{
		ID:          createdUser.ID,
...
func (h *Handler) updateManagedUser(c *gin.Context) {
	...
	if !admin {
		c.JSON(http.StatusForbidden, gin.H{"error": "Only circle admins can edit managed accounts"})
		return
	}

	managedID := 0
	if _, err := fmt.Sscanf(c.Param("id"), "%d", &managedID); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid user ID"})
		return
	}

	if _, err := h.isManagedInCircle(c, managedID, currentUser.CircleID); err != nil {
	...
	if req.Username != "" && !utils.IsValidUsername(req.Username) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid username format"})
		return
	}

	// Update username / displayName
	if err := h.userRepo.UpdateManagedUser(c, managedID, req.Username, req.DisplayName); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update managed user"})
		return
	}
	...
	if req.Password != "" {
		if len(req.Password) < 8 || len(req.Password) > 64 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Password must be 8–64 characters"})
			return
		}
		encodedPassword, err := auth.EncodePassword(req.Password)
		if err != nil {
		...
		if err := h.userRepo.UpdatePasswordByUserId(c.Request.Context(), managedID, encodedPassword); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update password"})
			return
		}
	}

	c.JSON(http.StatusOK, gin.H{"message": "Managed user updated successfully"})
}
...
func (h *Handler) deleteManagedUser(c *gin.Context) {
	...
	if !admin {
		c.JSON(http.StatusForbidden, gin.H{"error": "Only circle admins can delete managed accounts"})
		return
	}
	...
	result, err := h.deletionService.DeleteUserAccount(c.Request.Context(), managedID, nil)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete managed user: " + err.Error()})
		return
	}
	if !result.Success {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete managed user"})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Managed user deleted successfully"})
}

func (h *Handler) getManagedUsers(c *gin.Context) {
	currentUser, ok := auth.CurrentUser(c)
	if !ok {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	...
	if !admin {
		c.JSON(http.StatusForbidden, gin.H{"error": "Only circle admins can view managed accounts"})
		return
	}

	users, err := h.userRepo.GetManagedUsers(c, currentUser.CircleID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get managed users"})
		return
	}
...
func Routes(router *gin.Engine, h *Handler, jwtAuth *jwt.GinJWTMiddleware, limiter *limiter.Limiter, cfg *config.Config) {
	...
	userRoutes.Use(jwtAuth.MiddlewareFunc(), utils.RateLimitMiddleware(limiter))
	{
		...
		// Child user management endpoints
		userRoutes.POST("/subaccounts", h.createManagedUser)
		userRoutes.GET("/subaccounts", h.getManagedUsers)
		userRoutes.PUT("/subaccounts/:id", h.updateManagedUser)
		userRoutes.DELETE("/subaccounts/:id", h.deleteManagedUser)
	}

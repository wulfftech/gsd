...
// GetOverdueRecurringChores returns all active recurring chores whose next_due_date is in the past.
// Chores pending approval are excluded as they require human review.
// The Assignees relation is preloaded so that scheduling logic can use it.
func (r *ChoreRepository) GetOverdueRecurringChores(c context.Context) ([]*chModel.Chore, error) {
	var chores []*chModel.Chore
	nonRecurring := []string{
		string(chModel.FrequencyTypeOnce),
		string(chModel.FrequencyTypeNoRepeat),
		string(chModel.FrequencyTypeTrigger),
	}
	err := r.db.WithContext(c).
		Where("is_active = ? AND next_due_date < ? AND frequency_type NOT IN ? AND status != ?",
			true, time.Now().UTC(), nonRecurring, chModel.ChoreStatusPendingApproval).
		Preload("Assignees").
		Find(&chores).Error
	if err != nil {
		return nil, err
...
// MarkChoreAsMissed records a Missed history entry for the chore's current due date and
// advances next_due_date to nextDueDate (nil deactivates the chore).
// CompletedBy is set to 0 to indicate a system-generated record.
func (r *ChoreRepository) MarkChoreAsMissed(c context.Context, chore *chModel.Chore, nextDueDate *time.Time) error {
	return r.db.WithContext(c).Transaction(func(tx *gorm.DB) error {
		missedAt := time.Now().UTC()
		ch := &chModel.ChoreHistory{
			ChoreID:     chore.ID,
			PerformedAt: &missedAt,
			CompletedBy: 0, // system
			AssignedTo:  chore.AssignedTo,
			DueDate:     chore.NextDueDate,
		...
		return tx.Model(&chModel.Chore{}).Where("id = ?", chore.ID).Updates(updates).Error
	})
}

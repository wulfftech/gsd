// Package scheduling provides pure scheduling logic for chore due dates.
// It is extracted from package chore so that other packages (e.g. notifier)
// can compute next-due-dates without creating a circular import.
package scheduling
...
// ScheduleNextDueDate calculates the next due date for a chore after it has
// been completed (or skipped/missed) at completedDate.
// Returns nil for non-recurring frequency types ("once", "no_repeat", "trigger").
...
// ScheduleAdaptiveNextDueDate calculates the next due date using weighted
// historical completion intervals.

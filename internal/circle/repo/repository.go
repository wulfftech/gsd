...
func (r *CircleRepository) DepositPoints(c context.Context, circleID int, userID int, points int, createdBy int) error {
	logger := logging.FromContext(c)
	err := r.db.Transaction(func(tx *gorm.DB) error {

		if err := tx.Model(&cModel.UserCircle{}).Where("user_id = ? AND circle_id = ?", userID, circleID).Update("points", gorm.Expr("points + ?", points)).Error; err != nil {
			return err
		}
		if err := tx.Create(&pModel.PointsHistory{
	...
	if err != nil {
		logger.Error("Error depositing points", err)
		return err
	}
	return nil
}

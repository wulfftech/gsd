...
import { DepositPoints } from '../../utils/Fetcher.jsx'
import { resolvePhotoURL } from '../../utils/Helpers.jsx'
import DepositPointsModal from '../Modals/DepositPointsModal'
import RewardsPickerModal from '../Modals/RewardsPickerModal'
const UserPoints = () => {
  const [tabValue, setTabValue] = useState('now')
  ...
  useEffect(() => {
    if (!choresHistoryData || tabValue === 'now') return
    var history
    if (tabValue === 7) {
  ...
  const calculateLeaderboard = () => {
    ...
    if (tabValue === 'now') {
      ...
      sortField = leaderboardMode === 'points' ? 'periodPoints' : 'periodTasks'
    }
            ...
            <Stack sx={{ flex: 1 }}>
              ...
              <Typography level='body-sm' sx={{ color: 'text.secondary' }}>
                {tabValue === 'now'
                  ...
                  : `Rankings based on ${leaderboardMode === 'points' ? 'points earned' : 'tasks completed'} during the selected time period`}
              </Typography>
            </Stack>
          </Box>
            ...
            >
              <Tabs
                onChange={(e, newTabValue) => {
                  ...
                  if (newTabValue !== 'now') {
                    handleChoresHistoryLimitChange(newTabValue)
                  }
                }}
                value={tabValue}
                size='sm'
        ...
        <Stack spacing={2}>
          ...
          >
            ...
            <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1 }}>
              ...
              {/* Deposit — admin only */}
              {circleUsers.find(user => user.userId === userProfile.id)
                ?.role === 'admin' && (
                <Button
                  variant='soft'
                  color='success'
      ...
      <RewardsPickerModal
        config={(() => {
          ...
          return {
            ...
            onRedeemed: () => handleCircleMembersRefetch(),
        ...
        config={(() => {
          ...
          return {
            ...
            onSave: ({ userId, points }) => {
              DepositPoints(userId, points, userProfile.circleID)
                .then(() => {
                  setIsDepositModalOpen(false)
